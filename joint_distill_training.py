...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="SG161222/Realistic_Vision_V4.0",
        help=(
            'Custom noise scheduler to be loaded with pre-trained model'
        ),
    )
...
def main():
    ...
    noise_scheduler = DDPMScheduler.from_pretrained(args.noise_scheduler, subfolder="scheduler")
    logger.info(f"Teacher scheduler train timesteps={noise_scheduler.config.num_train_timesteps}")
    ...
    prog_distiller = DistillationPipeline()

    num_sample_steps = noise_scheduler.config.num_train_timesteps
    current_sample_steps = num_sample_steps
    end_num_steps = 4 # eventual number of sampling steps we want to use 

    while current_sample_steps > end_num_steps:
        noise_scheduler = DDPMScheduler.from_pretrained(args.noise_scheduler, subfolder="scheduler")
        
        file_path = args.noise_scheduler+"/scheduler/scheduler_config.json"

        with open(file_path, 'r', encoding='utf-8') as json_file:
            scheduler_json = json.load(json_file)

        num_sample_steps = scheduler_json['num_train_timesteps']
        current_sample_steps = num_sample_steps // 2
        scheduler_json['num_train_timesteps'] = current_sample_steps

        with open(file_path, 'w', encoding='utf-8') as json_file:
            json.dump(scheduler_json, json_file)

        current_scheduler = DDPMScheduler.from_pretrained(args.noise_scheduler, subfolder="scheduler")
        

        # train the student against the teacher model
        print('distilling teacher using %d sampling steps into student using %d steps'
            % (num_sample_steps, num_sample_steps // 2))

        # # Get the text embedding for conditioning
        # encoder_hidden_states = text_encoder(batch["input_ids"])[0]

        KD_teacher_unet, distilled_ema, distill_accelrator = prog_distiller(KD_teacher_unet, num_sample_steps, train_dataset, epochs=3, batch_size=128, accelerator=accelerator, train_dataloader=train_dataloader, weight_dtype=weight_dtype, vae=vae, text_encoder=text_encoder, noise_scheduler=current_scheduler)

        print("Progressive Distillation run successfully****************")

        noise_scheduler = current_scheduler
